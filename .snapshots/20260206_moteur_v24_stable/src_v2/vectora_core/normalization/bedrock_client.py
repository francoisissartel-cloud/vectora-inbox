...
class BedrockNormalizationClient:
    ...
    def invoke_with_prompt(
        ...
        Args:
            prompt_template: Template de prompt (lai_domain_scoring.yaml) OU string prompt flat
            context: Variables de contexte (item_title, item_summary, etc.)
            domain_definition: Définition du domaine (lai_domain_definition.yaml) - DEPRECATED
        
        Returns:
            Réponse texte de Bedrock
        """
        # V3: Support prompt flat (string) OU template YAML (dict)
        if isinstance(prompt_template, str):
            # Prompt flat v3: substituer variables dans le prompt
            full_prompt = prompt_template
            for key, value in context.items():
                full_prompt = full_prompt.replace(f"{{{{{key}}}}}", str(value))
            logger.info("Using flat prompt v3 (no references)")
        else:
            # Legacy: template YAML avec références dynamiques
            scopes_with_domain = dict(self.canonical_scopes) if self.canonical_scopes else {}
            ...
            logger.info("Using legacy YAML prompt template")
        
        request_body = {
            "anthropic_version": "bedrock-2023-05-31",
            "max_tokens": 1500 if isinstance(prompt_template, str) else prompt_template.get('bedrock_config', {}).get('max_tokens', 1500),
            "messages": [
                {
                    "role": "user",
                    "content": full_prompt
        ...
        response_text = call_bedrock_with_retry(self.model_id, request_body)
        logger.info(f"DEBUG invoke_with_prompt - Response length: {len(response_text)}")
        logger.info(f"DEBUG invoke_with_prompt - Response preview: {response_text[:500]}")
        return response_text