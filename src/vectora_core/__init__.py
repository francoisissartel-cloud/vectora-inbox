...
def run_ingest_normalize_for_client(
    ...
    logger.info(f"Period days résolu pour l'ingestion : {resolved_period_days} jours (payload: {period_days})")
    ...
    logger.info(f"Filtre temporel : items antérieurs au {cutoff_date_str} seront ignorés")
    ...
    # Étape 5 : Filtre temporel AVANT normalisation
    logger.info("Application du filtre temporel sur les items bruts")
    ...
    # Étape 6 : Normalisation (uniquement sur les items filtrés)
    logger.info("Phase 1B : Normalisation des items filtrés avec Bedrock")
    
    # Extraire les watch_domains pour l'évaluation de domaines
...
def run_engine_for_client(
    ...
    watch_domains = client_config.get('watch_domains', [])
    matched_items = matcher.match_items_to_domains(filtered_items, watch_domains, canonical_scopes, matching_rules)
    ...
    # Utiliser le nouveau système domain_relevance si disponible
    use_domain_relevance = any(item.get('domain_relevance') for item in matched_items)
    if use_domain_relevance:
        logger.info("Utilisation du système domain_relevance pour le scoring")
    else:
        logger.info("Utilisation du système de matching traditionnel pour le scoring")
    
    scored_items = scorer.score_items(matched_items, scoring_rules, watch_domains, canonical_scopes, use_domain_relevance)
    ...
    result = {
        ...
        'items_after_exclusions': len(filtered_items),
        'items_excluded': exclusion_stats['excluded_count'],
        'exclusion_rate': exclusion_stats['exclusion_rate'],
...
def _apply_temporal_filter(raw_items: List[Dict[str, Any]], cutoff_date_str: str) -> List[Dict[str, Any]]:
    """
    Filtre les items bruts selon leur date de publication.
    
    Args:
        raw_items: Liste d'items bruts avec champ 'published_at'
        cutoff_date_str: Date de coupure au format YYYY-MM-DD
    
    Returns:
        Liste d'items dont la date >= cutoff_date
    """
    import logging
    ...
    for item in raw_items:
        ...
        if not published_at:
            ...
            logger.debug(f"Item sans date ignoré : {item.get('title', '')[:50]}...")
            continue
        ...
        try:
            ...
            items_without_date += 1
            logger.debug(f"Item avec date invalide ignoré ({published_at}) : {item.get('title', '')[:50]}... - {e}")
