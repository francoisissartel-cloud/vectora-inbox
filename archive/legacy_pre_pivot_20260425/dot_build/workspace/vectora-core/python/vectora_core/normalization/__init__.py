...
def run_normalize_score_for_client(
    ...
    try:
        ...
        # 6. Items déjà scorés par Bedrock (domain_score) - pas de re-scoring Python
        scored_items = matched_items
        logger.info(f"Items prêts pour newsletter: {len(scored_items)} (scoring Bedrock uniquement)")
