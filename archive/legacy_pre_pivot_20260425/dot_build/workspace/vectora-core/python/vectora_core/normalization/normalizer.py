...
def _normalize_sequential(
    ...
    for i, item in enumerate(raw_items):
        try:
            ...
            # Construction de l'item temporaire pour le domain scoring
                ...
                try:
                    ...
                    if domain_definition:
                        ...
                        if domain_scoring_prompt:
                            domain_scoring_result = score_item_for_domain(temp_item, domain_definition, canonical_scopes, bedrock_client, domain_scoring_prompt, s3_io=s3_io, config_bucket=config_bucket)
            ...
            # Enrichissement de l'item avec les résultats (normalisation + domain scoring)
            normalized_item = _enrich_item_with_normalization(item, normalization_result, domain_scoring_result=domain_scoring_result)
...
def _enrich_item_with_normalization(
    ...
    NOUVEAU: Support domain scoring uniquement (matching supprimé).
    """
    ...
    # Extraction et validation des dates Bedrock
    extracted_date = normalization_result.get('extracted_date')  # Date de l'événement
    published_date_bedrock = normalization_result.get('published_date')  # Date de publication (Bedrock)
    date_confidence = normalization_result.get('date_confidence', 0.0)
    
    if extracted_date:
        try:
            datetime.strptime(extracted_date, '%Y-%m-%d')
            logger.info(f"Event date extracted by Bedrock: {extracted_date} (confidence: {date_confidence})")
        except ValueError:
            logger.warning(f"Invalid event date format from Bedrock: {extracted_date}")
            extracted_date = None
            date_confidence = 0.0
    
    if published_date_bedrock:
        try:
            datetime.strptime(published_date_bedrock, '%Y-%m-%d')
            logger.info(f"Published date extracted by Bedrock: {published_date_bedrock}")
        except ValueError:
            logger.warning(f"Invalid published date format from Bedrock: {published_date_bedrock}")
            published_date_bedrock = None
    
    # Calcul de effective_date (date unique pour tout le pipeline)
    published_at = original_item.get('published_at', '')
    
    # STRATEGIE: Prioriser Bedrock si haute confiance, fallback sur published_at
    if extracted_date and date_confidence > 0.7:
        # Cas 1: Bedrock a extrait avec confiance élevée
        effective_date = extracted_date
        date_source = 'bedrock_event_date'
        
        if published_at and published_at[:10] != extracted_date:
            logger.info(f"Using Bedrock date {extracted_date} over published_at {published_at[:10]} (confidence: {date_confidence})")

    elif published_at:
        # Cas 2: Fallback sur published_at si Bedrock échoue
        effective_date = published_at[:10] if len(published_at) >= 10 else published_at
        date_source = 'published_at'
        logger.debug(f"Using published_at {effective_date} (Bedrock confidence too low: {date_confidence})")

    else:
        # Cas 3: Aucune date disponible (utiliser date d'ingestion)
        effective_date = datetime.now().strftime('%Y-%m-%d')
        date_source = 'fallback'
        logger.warning(f"No date found, using ingestion date: {effective_date}")
    
    # Ajouter au niveau racine
    enriched_item['effective_date'] = effective_date
    enriched_item['date_metadata'] = {
        'source': date_source,
        'bedrock_event_date': extracted_date,
        'bedrock_published_date': published_date_bedrock,
    ...
    # NOUVEAU: Ajout des résultats de domain scoring (appel Bedrock lai_domain_scoring.yaml)
