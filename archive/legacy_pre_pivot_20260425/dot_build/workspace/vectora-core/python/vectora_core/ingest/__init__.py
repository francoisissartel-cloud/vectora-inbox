...
Workflow d'ingestion (10 étapes):
1. Chargement configs (client_config, source_catalog, scopes)
2. Résolution sources (bouquets → source_keys)
3. Initialisation scopes (exclusions, companies, LAI keywords)
4. Boucle sources: fetch → parse → extract dates → enrich
5. Filtre temporel (period_days)
6. Nettoyage contenu (footers/headers)
7. Application profils ingestion (pure/hybrid/press)
8. Déduplication (content_hash)
9. Validation items
10. Écriture S3 (ingested/)
"""
...
def run_ingest_for_client(
    ...
    try:
        ...
        # Initialiser parsing config depuis S3
        logger.info("Étape 2.8 : Initialisation parsing config depuis S3")
        content_parser.initialize_parsing_config(s3_io, config_bucket)
        ...
        # Étape 4 : Ingestion et parsing (PARALLÈLE)
        logger.info("Phase 1A : Ingestion des sources (traitement parallèle)")
        all_raw_items = []
        sources_processed = 0
        sources_failed = 0
        
        # Traitement parallèle des sources (max 3 workers)
        max_workers = min(3, len(resolved_sources))
        logger.info(f"Traitement parallèle avec {max_workers} workers")
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_source = {
                executor.submit(_process_single_source, source_meta): source_meta
                for source_meta in resolved_sources
            }
            
            for future in as_completed(future_to_source):
                source_meta = future_to_source[future]
                ...
                try:
                    ...
                    if result and result.get('items'):
                        ...
                        sources_processed += 1
                        logger.info(f"Source {source_key} : {len(result['items'])} items parsés")
                    else:
                        sources_failed += 1
                        logger.warning(f"Source {source_key} : {result.get('error', 'Aucun item')}")
                except Exception as e:
                    sources_failed += 1
                    logger.error(f"Source {source_key} : erreur - {e}")
        
        logger.info(f"Total items parsés : {len(all_raw_items)} depuis {sources_processed} sources")
        if sources_failed > 0:
            logger.warning(f"Sources en échec : {sources_failed}")
        
        # Métriques CloudWatch structurées
        _log_cloudwatch_metrics({
            'client_id': client_id,
            'sources_processed': sources_processed,
            'sources_failed': sources_failed,
            'items_parsed': len(all_raw_items)
        })
        
        # Étape 5 : Filtre temporel (AVANT profil pour économiser)
        logger.info("Étape 5 : Filtre temporel sur published_at")
        from_date_calc, to_date_calc = utils.compute_date_range(resolved_period_days, from_date, to_date)
        cutoff_date = from_date_calc
        logger.info(f"Cutoff date : {cutoff_date} (period_days={resolved_period_days})")
        
        temporal_filtered = []
        items_excluded_by_date = 0
        items_without_date = 0
        
        for item in all_raw_items:
            published_at = item.get('published_at')
            # Garder l'item si published_at est vide/null OU si >= cutoff_date
            if not published_at:
                temporal_filtered.append(item)
                items_without_date += 1
            elif published_at >= cutoff_date:
                temporal_filtered.append(item)
            else:
                items_excluded_by_date += 1
                logger.debug(f"EXCLU (date): {published_at} < {cutoff_date}")
        
        logger.info(f"Filtre temporel : {len(temporal_filtered)}/{len(all_raw_items)} conservés ({items_excluded_by_date} exclus, {items_without_date} sans date)")
        
        # Étape 5.5 : Nettoyage et enrichissement du contenu (ASYNC)
        logger.info("Étape 5.5 : Nettoyage du contenu (retire footers/headers)")
        cleaned_items = content_cleaner.clean_and_enrich_items(temporal_filtered)
        logger.info(f"Contenu nettoyé pour {len(cleaned_items)} items")
        
        # Enrichissement asynchrone (si items avec contenu court)
        logger.info("Étape 5.6 : Enrichissement asynchrone du contenu")
        try:
            from .content_extractor import ContentExtractor
            extractor = ContentExtractor()
            cleaned_items = asyncio.run(extractor.enrich_items_async(cleaned_items))
            enriched_count = sum(1 for item in cleaned_items if item.get('enriched', False))
            logger.info(f"Enrichissement terminé : {enriched_count} items enrichis")
        except Exception as e:
            logger.warning(f"Enrichissement asynchrone échoué : {e}")
        
        # Étape 6 : Profil d'ingestion (LAI keywords + exclusions)
        logger.info("Étape 6 : Profil d'ingestion (LAI keywords + exclusions)")
        profile_filtered = []
        for item in cleaned_items:
            source_meta = next((s for s in resolved_sources if s['source_key'] == item['source_key']), {})
            filtered = ingestion_profiles.apply_ingestion_profile([item], source_meta, ingestion_mode)
            profile_filtered.extend(filtered)
        
        items_excluded_by_profile = len(cleaned_items) - len(profile_filtered)
        logger.info(f"Profil : {len(profile_filtered)}/{len(cleaned_items)} conservés ({items_excluded_by_profile} exclus)")
        
        # Étape 7 : Déduplication
        logger.info("Étape 7 : Déduplication par content_hash")
        deduplicated_items = utils.deduplicate_items(profile_filtered)
        items_deduplicated = len(profile_filtered) - len(deduplicated_items)
        logger.info(f"Déduplication : {len(deduplicated_items)} uniques ({items_deduplicated} doublons)")
        
        # Étape 8 : Validation
        logger.info("Étape 8 : Validation des items")
        valid_items = [item for item in deduplicated_items if utils.validate_item(item)]
        items_invalid = len(deduplicated_items) - len(valid_items)
        logger.info(f"Validation : {len(valid_items)} valides ({items_invalid} invalides)")
        
        # Stats extraction de dates
        if valid_items:
            date_sources = {}
            for item in valid_items:
                source = item.get('date_extraction', {}).get('date_source', 'unknown')
                date_sources[source] = date_sources.get(source, 0) + 1
            logger.info(f"Date extraction stats: {date_sources}")
        ...
        result = {
            ...
            'sources_failed': sources_failed,
            'items_parsed': len(all_raw_items),
            'items_excluded_by_date': items_excluded_by_date,
            'items_excluded_by_profile': items_excluded_by_profile,
...
def _process_single_source(source_meta: Dict[str, Any]) -> Dict[str, Any]:
    """
    Traite une source unique (fetch + parse).
    Fonction helper pour traitement parallèle.
    
    Args:
        source_meta: Métadonnées de la source
    
    Returns:
        Dict avec items parsés ou erreur
    """
    source_key = source_meta.get('source_key')
...
def _log_cloudwatch_metrics(metrics: Dict[str, Any]) -> None:
    """
    Log métriques au format CloudWatch Embedded Metrics.
    
    Args:
        metrics: Dictionnaire de métriques à logger
    """
    # Format CloudWatch Embedded Metrics Format (EMF)
    emf_log = {
        "_aws": {
            "Timestamp": int(time.time() * 1000),
            "CloudWatchMetrics": [{
                "Namespace": "VectoraInbox/Ingestion",
                "Dimensions": [["ClientId"]],
                "Metrics": []
            }]
        },
        "ClientId": metrics.get('client_id', 'unknown')
    }
    
    # Ajouter métriques numériques
    metric_definitions = []
    for key, value in metrics.items():
        if key != 'client_id' and isinstance(value, (int, float)):
            emf_log[key] = value
            metric_definitions.append({
                "Name": key,
                "Unit": "Count" if key.startswith('items_') or key.startswith('sources_') else "Seconds"
            })
    
    emf_log["_aws"]["CloudWatchMetrics"][0]["Metrics"] = metric_definitions
    
    # Log au format JSON pour CloudWatch
