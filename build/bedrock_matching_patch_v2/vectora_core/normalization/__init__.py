...
def run_normalize_score_for_client(
    ...
    try:
        ...
        # 3. Chargement des items ingérés
        ...
        if not raw_items:
            logger.warning("Aucun item trouvé dans le dernier run")
            return {
                "client_id": client_id,
                "status": "completed",
                "items_processed": 0,
        ...
        # NOUVEAU: Récupération des watch_domains pour le matching Bedrock
        ...
        # 5. Matching aux domaines de veille (déterministe + Bedrock déjà fait)
