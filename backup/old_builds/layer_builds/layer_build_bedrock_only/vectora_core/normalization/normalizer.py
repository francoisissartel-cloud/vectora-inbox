...
def _normalize_sequential(
    ...
    for i, item in enumerate(raw_items):
        try:
            ...
            # NOUVEAU: Matching Bedrock par domaines
            bedrock_matching_result = {}
            if watch_domains and canonical_scopes:
                try:
                    from .bedrock_matcher import match_watch_domains_with_bedrock
                    
                    # Construire l'item pour le matching
                    item_for_matching = {
                        ...
                        'event_type': normalization_result.get('event_type', 'other')
                    }
                    
                    # Appel Bedrock matching (ALIGNÉ sur même config que normalisation)
                    bedrock_matching_result = match_watch_domains_with_bedrock(
                        item_for_matching, watch_domains, canonical_scopes, matching_config
                    )
                    
                    logger.debug(f"Matching Bedrock V2: {len(bedrock_matching_result.get('matched_domains', []))} domaines")
                    
                except Exception as e:
                    logger.warning(f"Erreur matching Bedrock V2: {e}")
